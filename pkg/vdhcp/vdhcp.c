 ...
 *
 * Usage: vdhcp [-1] [-i] [-r] [-h HOSTNAME] [-c CONF] [INTERFACE...]
 *   -1          one-shot: exit once bound (don't stay to renew)
 *   -i          DHCPINFORM: fetch options for a statically-configured address
 *   -r          try RFC 4039 rapid commit (2-message lease)
...
struct conf {
    ...
    uint32_t metric;            /* default-route metric (0 = kernel default) */
    int      resolv;            /* 1 = manage /etc/resolv.conf (default), 0 = leave alone */
    int      rapid;             /* 1 = try RFC 4039 rapid commit */
    int      inform;            /* 1 = DHCPINFORM mode (options only, keep static addr) */
};
static struct conf g_conf;

#define RESOLV_MARK "# generated by vdhcp\n"
...
static void load_conf(const char *path) {
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        *eq = 0;
        ...
        else if (!strcmp(k, "metric")) g_conf.metric = (uint32_t)strtoul(v, NULL, 10);
        else if (!strcmp(k, "resolv")) g_conf.resolv = (!strcmp(v, "no") || !strcmp(v, "0")) ? 0 : 1;
        else if (!strcmp(k, "rapid"))  g_conf.rapid = (!strcmp(v, "yes") || !strcmp(v, "1")) ? 1 : 0;
        else if (!strcmp(k, "inform")) g_conf.inform = (!strcmp(v, "yes") || !strcmp(v, "1")) ? 1 : 0;
...
static uint8_t *opt(uint8_t *p, uint8_t code, uint8_t len, const void *val) {
    *p++ = code; *p++ = len; if (len) memcpy(p, val, len); return p + len;
}
...
static int build(struct dhcp_msg *m, int type, uint32_t xid,
    ...
    if (type == DHCP_DISCOVER && g_conf.rapid) p = opt(p, 80, 0, NULL);  /* RFC 4039 rapid commit */
    if (type == DHCP_DISCOVER || type == DHCP_REQUEST || type == DHCP_INFORM) {
        ...
        if (g_conf.req_lease && type != DHCP_INFORM) { uint32_t v = htonl(g_conf.req_lease); p = opt(p, 51, 4, &v); }
...
static void add_route(uint32_t dest, uint32_t mask, uint32_t gw, uint32_t metric) {
    ...
    if (metric) rt.rt_metric = (short)(metric + 1);   /* SIOCADDRT stores metric+1 */
    rt.rt_dev = (char *)g_if;
    if (ioctl(g_cfg, SIOCADDRT, &rt) < 0 && errno != EEXIST) logmsg("route: %s", strerror(errno));
}

/* true if /etc/resolv.conf is absent, empty, or one we generated — i.e. safe to
 * overwrite. A non-empty file without our marker is treated as admin-managed. */
static int resolv_managed(void) {
    FILE *f = fopen("/etc/resolv.conf", "r");
    if (!f) return 1;
    char first[64] = ""; char *got = fgets(first, sizeof first, f);
    fclose(f);
    if (!got) return 1;
    return strncmp(first, RESOLV_MARK, strlen(RESOLV_MARK)) == 0;
}
static void write_resolv(const struct lease *ls) {
    if (!g_conf.resolv || !resolv_managed()) return;
...
/* apply a lease; set_addr=0 applies options only (DHCPINFORM, static address) */
static void apply_lease(const struct lease *ls, int set_addr) {
    if (set_addr) {
        set_ifaddr(ls->ip, ls->mask ? ls->mask : htonl(0xffffff00));
        if (ls->bcast) set_bcast(ls->bcast);
    }
    if (ls->mtu)   set_mtu(ls->mtu);
    if (ls->router) add_route(0, 0, ls->router, g_conf.metric);       /* default route */
    for (int i = 0; i < ls->nsroute; i++) {                           /* classless static routes */
        uint32_t w = ls->sroute[i].width;
        uint32_t mask = w ? htonl(~((w >= 32) ? 0u : ((1u << (32 - w)) - 1))) : 0;
        add_route(ls->sroute[i].dest, mask, ls->sroute[i].gw, g_conf.metric);
    }
    write_resolv(ls);
    ...
    char b[16];
    logmsg("%s %s/%d lease %us%s%s", set_addr ? "leased" : "informed", ip4(ls->ip, b),
           __builtin_popcount(ls->mask), ls->lease,
           ls->nsroute ? " +routes" : "", ls->mtu ? " +mtu" : "");
}
...
/* read the interface's currently-configured IPv4 address (0 if none) */
static uint32_t get_ifaddr(void) {
    struct ifreq ifr; memset(&ifr, 0, sizeof ifr);
    snprintf(ifr.ifr_name, IFNAMSIZ, "%s", g_if);
    if (ioctl(g_cfg, SIOCGIFADDR, &ifr) < 0) return 0;
    return ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr.s_addr;
}

/* Wait `secs` while BOUND, watching the ARP socket for a conflict (RFC 5227):
 * another host sourcing our address with a different MAC. Returns 0 = timer
 * expired, 1 = SIGTERM, 2 = conflict detected. */
static int bound_wait(int arp, uint32_t myip, unsigned secs) {
    if (arp < 0) return nap(secs) ? 1 : 0;
    time_t deadline = time(NULL) + secs;
    while (!g_term) {
        long rem = (long)deadline - (long)time(NULL);
        if (rem <= 0) return 0;
        int w = wait_readable(arp, rem > 3600 ? 3600000 : (int)(rem * 1000));
        if (w == -2) return 1;
        if (w == 0) {
            struct arp_pkt r;
            if (recv(arp, &r, sizeof r, 0) >= (int)sizeof r &&
                memcmp(r.spa, &myip, 4) == 0 && memcmp(r.sha, g_mac, 6) != 0)
                return 2;
        }
    }
    return 1;
...
static int run_interface(const char *ifname) {
    ...
    int arp = arp_open();                       /* persistent, for RFC 5227 defense */

    /* DHCPINFORM mode: fetch options for a statically-configured address, apply
     * options only (never the address), and re-inform periodically. */
    if (g_conf.inform) {
        while (!g_term) {
            uint32_t myip = get_ifaddr();
            if (!myip) { logmsg("INFORM needs a configured address"); if (nap(5)) break; continue; }
            struct dhcp_msg m, r; struct lease ls;
            uint32_t xid = (uint32_t)rand();
            int len = build(&m, DHCP_INFORM, xid, myip, 0, 0, 1);
            int n = retransmit(0, &m, len, htonl(INADDR_BROADCAST), 1, udp, &r, xid, 30);
            if (n == -2) break;
            if (n == -2) break;
            if (n > 0 && parse(&r, n, &ls) == DHCP_ACK) {
                ls.ip = myip;
                apply_lease(&ls, 0);
                run_hook("INFORM", &ls);
                if (g_oneshot) break;
                ...
                logmsg("no INFORM reply");
                if (g_oneshot) break;               /* don't loop forever in one-shot */
                if (nap(10)) break;
            }
        }
        if (arp >= 0) close(arp);
    ...
    while (!g_term) {
        ...
        if (try_reboot) {
            ...
            type = parse(&r, n, &ls);
            if (type == DHCP_ACK && ls.ip) {
                logmsg("rapid-commit");             /* RFC 4039: committed, skip REQUEST */
            } else if (type == DHCP_OFFER && ls.ip) {
        ...
        int released = 0;
        for (;;) {
            int bw = bound_wait(arp, cur.ip, cur.t1);   /* sleep to T1, defending the address */
            if (bw == 1) { released = 1; break; }       /* SIGTERM */
            if (bw == 2) {                              /* another host claimed our address */
                char b[16]; logmsg("address conflict on %s, declining", ip4(cur.ip, b));
                send_decline(pkt, &cur); run_hook("EXPIRE", &cur);
                deconfigure(); forget_lease(); bound = 0; break;
            }
            xid = (uint32_t)rand();
            ...
            if (n > 0 && parse(&r, n, &ls) == DHCP_ACK && ls.ip) {
                inherit(&ls, &cur); set_timers(&ls); apply_lease(&ls, 1); save_lease(&ls);
...
int main(int argc, char **argv) {
    ...
    char cli_host[128] = "";
    int cli_inform = 0, cli_rapid = 0;
    g_conf.resolv = 1;                          /* manage resolv.conf unless told otherwise */
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-1")) g_oneshot = 1;
        else if (!strcmp(argv[i], "-i")) cli_inform = 1;
        else if (!strcmp(argv[i], "-r")) cli_rapid = 1;
    ...
    if (cli_inform) g_conf.inform = 1;
    if (cli_rapid)  g_conf.rapid = 1;
